...
package org.puremvc.as3.multicore.utilities.pipes.plumbing
{
	...
	public class Queue extends Pipe
	{
		 ...
		 */
		override public function write( message:PipeMessage ):boolean
		{
		 ...
		 */
		protected function store( message:PipeMessage ):void
		{
		 ...
		 */
		protected function sortMessagesByPriority(msgA:PipeMessage, msgB:PipeMessage):Number
		{
			var num:Number = 0;
		...
		{
			var success:boolean=true;
			var message:PipeMessage = messages.shift() as PipeMessage;
			while ( message !== null )
			{
				var ok:boolean = output.write( message );
				if ( !ok ) success = false;
				message = messages.shift() as PipeMessage;
			}
			return success;
		}
